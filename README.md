...
> 这个标题下的是我 fork 之后的内容，下面的是原大佬作者的。

首先学习的动机是我开始实习之后，现在下班之后就什么都不想干了，只会不停的刷视频摆烂，感觉自己相比以前的那个热血（虽然也挺菜的） acmer 身份相比堕落了不少。所以给自己找了个有点有趣但又能学到东西的事情来做，那就是搭建属于自己的小破站（网站），上面随便弄点有趣的小游戏、整活小功能之类，这样不仅能做点自己感兴趣的东西，还能在过程中多用用 linux、docker、bazel、rpc 之类的语言或工具，稍微提升自己的开发能力。

当然我还是想着从最基本的框架搞起，毕竟还是觉得没有真正读懂过一个简单的框架就不可能真正用好任何框架。

但是！读了两三天发现自己才读了这个日志系统，当然把 timer 和 eventloop 也读了（能读这些也是还有点线程、锁的编程基础），不过还是太慢了，毕竟一周最多也就两天时间，要读完这个、再按自己的理解写一个感觉还是太久了。

所以中间就想着转变一下计划，比如不要追求完全解读框架，学学使用 gRPC 什么的，摸索个一两天看看会不会回报率高一些，这个的阅读任务就暂时先搁置了。

> 以上编辑于 2024.5.25。

今天是 6.22，犹豫了一个月，我还是回来了，果然这一个月来基本上就是在浪费时间，而秋招在即，我觉得肯定还是要有个个人项目比较合理。所以我决定了，还是把这个项目的代码给吃透（虽然好像是很多人不看好的做法），然后再外化为自己的项目（把这里学到的一些技巧用到自己的 webserver 上面，比如异步日志的实现，一些更好的封装技巧，比如这种 tinyxml 的配置技巧就是一种小技术栈）。冲冲冲！！！

算了，我还是弄跟着网课学吧，再慢也比这样猜代码快。。。
